...
MQTT_PORT = int(os.getenv('MQTT_PORT', 1883))  # Default port if not set
MQTT_TOPIC = os.getenv('MQTT_TOPIC')
MQTT_USER = os.getenv('MQTT_USERNAME')
MQTT_PASSWORD = os.getenv('MQTT_PASSWORD')
SERVER_URL = os.getenv('BACKEND_SERVER_URL')
CAMERA_CONFIG = os.getenv('CAMERA_CONFIGURE')  # Default to 0 if not set
...
class CameraHandler:
    ...
    def connect(self):
        try:
            ...
            # Optimize camera settings for Jetson Nano
...
class FaceDetectionSystem:
    def __init__(self):
        ...
        self.SERVER_URL = os.getenv('BACKEND_SERVER_URL')
        if not self.SERVER_URL:
            self.logger.error("SERVER_URL not found in environment variables")
            self.SERVER_URL = "http://localhost:8000"  # Default fallback URL
    ...
    def process_frame(self, frame):
        try:
            ...
            if faces and (current_time - self.last_capture_time >= self.CAPTURE_INTERVAL):
                ...
                # Process face in separate thread
                threading.Thread(target=self._handle_face_recognition,
                              args=(face_base64,),
                              daemon=True).start()
                
                self.last_capture_time = current_time

            # Draw rectangles only if needed
